  ...
  const handleSubmit = async (asDraft: boolean = false) => {
    ...
    try {
      ...
      const orderData = {
        ...
        delivery_date: formData.deliveryDate ? Math.floor(new Date(formData.deliveryDate).getTime() / 1000) : undefined,
      ...
      // Create order via Dolibarr API
      await orderApi.create(orderData as Parameters<typeof orderApi.create>[0]);

      alert(asDraft ? 'Sipariş taslak olarak kaydedildi' : 'Sipariş başarıyla oluşturuldu');
      navigate('/siparisler');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Sipariş oluşturulurken hata oluştu');
      ...
      <div className="space-y-6 animate-fadeIn">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Müşterilerinizden gelen siparişleri Dolibarr'a kaydedin
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => handleSelectChange('deliveryDate', e.target.value)}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                ...
                onChange={(e) => handleSelectChange('shippingAddress', e.target.value)}
                placeholder="Siparişin teslim edileceği adres..."
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Siparişe ait notlar..."
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                rows={3}
              />
            </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100">
          ...
          <div className="flex gap-3">
            <Button variant="secondary" onClick={() => navigate('/siparisler')}>
              İptal
            </Button>
            <Button variant="secondary" onClick={() => handleSubmit(true)} disabled={isSubmitting}>
              <Save className="w-4 h-4" />
